...
{registrations}        }}
			...
			code = NamespaceAnalisysFormat(methodSymbol, code);
...
" + code;
		...
		private static string NamespaceAnalisysFormat(ISymbol methodSymbol, string code)
		{
			if (methodSymbol.ContainingNamespace.IsGlobalNamespace)
			{
				code = ProcessIdentation(code);
			}
			else
			{
